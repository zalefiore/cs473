...
static inline fxpt_4_28 fxpt_mul(fxpt_4_28 a, fxpt_4_28 b) {
  return (fxpt_4_28)(((int64_t)a * (int64_t)b) >> FXPT_FRAC_BITS);
}
...
uint16_t calc_mandelbrot_point_soft(fxpt_4_28 cx, fxpt_4_28 cy, uint16_t n_max) {
  ...
  const fxpt_4_28 FOUR = (fxpt_4_28)4 << FXPT_FRAC_BITS;
  do {
    xx = fxpt_mul(x, x);
    yy = fxpt_mul(y, y);
    two_xy = 2 * fxpt_mul(x, y);
